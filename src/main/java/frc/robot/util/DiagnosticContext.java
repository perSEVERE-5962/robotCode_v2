...
public final class DiagnosticContext {
    ...
    public static synchronized String buildContext() {
        try {
            ...
            sb.append(String.format("Match: %s #%d @ %.1fs | ",
            ...
            sb.append(String.format("Voltage: %.1fV | Loop: %.1fms",
                safeGetVoltage(), safeGetLoopTime()));
            return sb.toString();
        } catch (Throwable t) {
    ...
    public static void recordFailure(String failureType, String details) {
        try {
            double timestamp = Timer.getFPGATimestamp();
            Logger.recordOutput("Diagnostics/FailureContext",
                failureType + ": " + details + " | " + buildContext());
    ...
    private static String safeGetClassName(Throwable t) {
        try { return (t != null) ? t.getClass().getName() : "Unknown"; }
        catch (Throwable ex) { return "Unknown"; }
    }
    ...
    private static String safeGetMessage(Throwable t) {
        ...
        } catch (Throwable ex) { return "No message"; }
    }

    private static String safeGetThreadName(Thread t) {
        try { return (t != null) ? t.getName() : "Unknown"; }
        catch (Throwable ex) { return "Unknown"; }
    }

    private static String safeGetMatchType() {
        try { return DriverStation.getMatchType().toString(); }
        catch (Throwable t) { return "Unknown"; }
    }

    private static int safeGetMatchNumber() {
        try { return DriverStation.getMatchNumber(); }
        catch (Throwable t) { return 0; }
    }

    private static double safeGetMatchTime() {
        try { return DriverStation.getMatchTime(); }
    ...
    private static double safeGetVoltage() {
        try { return RobotController.getBatteryVoltage(); }
        catch (Throwable t) { return 0; }
    }

    private static double safeGetLoopTime() {
        try { return RobotController.getFPGATime() % 1000000 / 1000.0; }
        catch (Throwable t) { return 0; }
    }
}
