...
/** Pre-match diagnostics: SAFE mode (sensors only, while disabled) and FULL mode (actuator tests, requires TEST mode). */
public class PreMatchDiagnostics {
    ...
    // Results
    public enum CheckResult { PASS, WARN, FAIL, SKIPPED }
    public enum DiagnosticMode { SAFE, FULL }
    ...
    /**
     * Run SAFE diagnostic checks (sensor-only, no motor commands).
     * Safe to run while disabled. Takes ~5-6 seconds (gyro + vision sampling).
     */
    ...
    /**
     * Run FULL diagnostic checks (includes actuator tests).
     * REQUIRES TEST MODE - motors will move!
    ...
    public boolean runFullChecks() {
        ...
        if (!DriverStation.isTest() || !DriverStation.isEnabled()) {
            ElasticUtil.sendError("Full Check Blocked",
                "Enable TEST mode first! Motors need to move.", 5000);
    ...
    private void checkBattery() {
        ...
        if (voltage < BATTERY_FAIL_V) {
            addResult("Battery", CheckResult.FAIL,
                    String.format("%.1fV - REPLACE BATTERY", voltage));
        } else if (voltage < BATTERY_WARN_V) {
            addResult("Battery", CheckResult.WARN,
                    String.format("%.1fV - Low, consider replacing", voltage));
        } else {
            addResult("Battery", CheckResult.PASS,
    ...
    private void checkCANBus() {
        ...
        if (busOffDelta > 0) {
            ...
            addResult("CANBus", CheckResult.WARN,
            ...
            addResult("CANBus", CheckResult.PASS,
    ...
    private void checkMissingDevices() {
        ...
        try { if (Shooter.getInstance() == null) missing.append("Shooter,"); }
        catch (Exception e) { missing.append("Shooter,"); }

        try { if (Indexer.getInstance() == null) missing.append("Indexer,"); }
        catch (Exception e) { missing.append("Indexer,"); }

        try { if (Intake.getInstance() == null) missing.append("Intake,"); }
        catch (Exception e) { missing.append("Intake,"); }

        try { if (IntakeActuator.getInstance() == null) missing.append("IntakeActuator,"); }
        catch (Exception e) { missing.append("IntakeActuator,"); }

        try { if (Hanger.getInstance() == null) missing.append("Hanger,"); }
        catch (Exception e) { missing.append("Hanger,"); }

        String missingStr = missing.length() > 0
    ...
    private void checkMotorTemps() {
        try {
            ...
            if (maxTemp > MOTOR_TEMP_WARN_C) {
                addResult("MotorTemps", CheckResult.WARN,
                        String.format("Max %.0f°C - allow cooling", maxTemp));
            } else {
                addResult("MotorTemps", CheckResult.PASS,
                        String.format("Max %.0f°C", maxTemp));
            }
        } catch (Exception e) {
    ...
    private void checkMotorFaults() {
        try {
            ...
            if (shooterFaults != 0) faults.append("Shooter(0x").append(Integer.toHexString(shooterFaults)).append("),");
            if (indexerFaults != 0) faults.append("Indexer(0x").append(Integer.toHexString(indexerFaults)).append("),");
            if (intakeFaults != 0) faults.append("Intake(0x").append(Integer.toHexString(intakeFaults)).append("),");

            Logger.recordOutput("Diagnostics/MotorFaults", faults.toString());

            if (faults.length() > 0) {
                addResult("MotorFaults", CheckResult.WARN,
    ...
    /**
     * Sample gyro drift over 2 seconds while stationary.
     * More reliable than single-instant read.
     */
    ...
    /**
     * Sample vision frame drops over 3 seconds.
     * Checks for consistent target detection when stationary.
     */
    ...
    /**
     * Check drive sensors without commanding motion.
     * Verifies encoders and module CAN health.
     */
    ...
    private void checkShooterSpinup() {
        try {
            ...
            if (!shooter.isAtSpeed() || shooterSpinupMs > SHOOTER_SPINUP_FAIL_MS) {
                addResult("ShooterSpinup", CheckResult.FAIL,
                    String.format("%.0fms - too slow or failed", shooterSpinupMs));
            } else if (shooterSpinupMs > SHOOTER_SPINUP_WARN_MS) {
                addResult("ShooterSpinup", CheckResult.WARN,
                    String.format("%.0fms - slow", shooterSpinupMs));
            } else {
                addResult("ShooterSpinup", CheckResult.PASS,
                    String.format("%.0fms to speed", shooterSpinupMs));
            }
        } catch (Exception e) {
            shooterSpinupMs = -1;
            Logger.recordOutput("Diagnostics/ShooterSpinupMs", shooterSpinupMs);
    ...
    private void checkIndexerCurrent() {
        try {
            ...
            if (indexerCurrentPeak < INDEXER_CURRENT_MIN_A) {
                addResult("IndexerCurrent", CheckResult.WARN,
                    String.format("%.1fA peak - motor may be disconnected", indexerCurrentPeak));
            } else {
                addResult("IndexerCurrent", CheckResult.PASS,
    ...
    /**
     * Command small drive motion and verify response.
     * Tests closed-loop control actually works.
     */
    private void checkDriveMotion() {
        try {
            ...
            swerve.drive(
                new edu.wpi.first.math.geometry.Translation2d(0.1, 0),
                0,
                false
            );
            ...
            if (!motionDetected) {
                ...
                addResult("DriveMotion", CheckResult.WARN,
                    String.format("%.0fms response - sluggish", responseTime));
            } else {
                addResult("DriveMotion", CheckResult.PASS,
                    String.format("%.0fms response", responseTime));
            }
        } catch (Exception e) {
    ...
    private void logResults() {
        ...
        for (DiagnosticCheck check : lastResults) {
            Logger.recordOutput("Diagnostics/PreMatch/" + check.name + "/Passed",
                    check.result == CheckResult.PASS);
            Logger.recordOutput("Diagnostics/PreMatch/" + check.name + "/Result",
                    check.result.name());
            Logger.recordOutput("Diagnostics/PreMatch/" + check.name + "/Message",
                    check.message);
        }

        StringBuilder summary = new StringBuilder();
        for (DiagnosticCheck check : lastResults) {
            String icon = switch (check.result) {
                case PASS -> "[OK]";
                case WARN -> "[WARN]";
                case FAIL -> "[FAIL]";
                case SKIPPED -> "[SKIP]";
            };
            summary.append(icon).append(" ").append(check.name).append(": ")
    ...
    private void sendNotification(String checkType) {
        ...
        if (failCount > 0) {
            ElasticUtil.sendError(checkType + " FAILED" + modeNote,
                    String.format("%d fail, %d warn, %d skip", failCount, warnCount, skippedCount), 10000);
        } else if (warnCount > 0) {
            ElasticUtil.sendWarning(checkType + modeNote,
                    String.format("%d warnings, %d skipped", warnCount, skippedCount), 5000);
        } else {
            ElasticUtil.sendInfo(checkType + " PASSED" + modeNote,
                    skippedCount > 0
                        ? String.format("All passed (%d skipped - enable TEST for full)", skippedCount)
                        : "All systems ready", 3000);
        }
    }
    ...
    public List<DiagnosticCheck> getLastResults() { return new ArrayList<>(lastResults); }
    public int getFailCount() { return failCount; }
    public int getWarnCount() { return warnCount; }
    public int getSkippedCount() { return skippedCount; }
    public DiagnosticMode getLastMode() { return lastMode; }
