...
public class PredictiveAlerts {
    ...
    public void update() {
        ...
        double maxTemp = Math.max(tm.getShooterTemperature(),
            Math.max(tm.getIndexerTemperature(),
     ...
     * Returns change per second using actual loop rate from TelemetryManager.
     */
    ...
    private void checkPredictiveAlerts() {
        ...
        if (predictedTimeToWarning > 0 && predictedTimeToWarning < PREDICTION_WARN_SECONDS) {
            if (!batteryAlertSent) {
                ElasticUtil.sendWarning("Battery Prediction",
                    String.format("Battery will hit warning in %.0fs at current drain", predictedTimeToWarning));
        ...
        if (predictedTimeToOverheat > 0 && predictedTimeToOverheat < PREDICTION_WARN_SECONDS) {
            if (!tempAlertSent) {
                ElasticUtil.sendWarning("Temp Prediction",
                    String.format("Shooter will overheat in %.0fs at current rate", predictedTimeToOverheat));
                tempAlertSent = true;
            }
        } else {
            tempAlertSent = false;
    ...
    public void log() {
        ...
        Logger.recordOutput("Predict/BatteryAtRisk",
            predictedTimeToWarning > 0 && predictedTimeToWarning < PREDICTION_WARN_SECONDS);
        Logger.recordOutput("Predict/TempAtRisk",
