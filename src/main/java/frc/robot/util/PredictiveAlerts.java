                ...
                Math.max(
                        ...
                        Math.max(
                                ...
                                Math.max(
                                        ...
                                        Math.max(
                                                tm.getIntakeActuatorTemperature(),
                                                tm.getAgitatorTemperature()))));
        ...
        if (predictedTimeToWarning > 0 && predictedTimeToWarning < PREDICTION_WARN_SECONDS) {
            if (!batteryAlertSent && !suppressBatteryNotify) {
                ElasticUtil.sendWarning(
                        "Battery Prediction",
                        String.format(
                                "Battery will hit warning in %.0fs at current drain",
                                predictedTimeToWarning));
        ...
        if (predictedTimeToOverheat > 0 && predictedTimeToOverheat < PREDICTION_WARN_SECONDS) {
            if (!tempAlertSent) {
                ElasticUtil.sendWarning(
                        "Temp Prediction",
                        String.format(
                                "Shooter will overheat in %.0fs at current rate",
                                predictedTimeToOverheat));
                tempAlertSent = true;
            }
        } else {
            tempAlertSent = false;
