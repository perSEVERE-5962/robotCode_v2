...
public class EventMarker {
    ...
    /**
     * Flush buffered events to log. Call once per cycle from TelemetryManager.
     * This ensures all events from a cycle are logged together as an array.
     */
    public static synchronized void flushCycleEvents() {
        try {
            if (droppedEventsThisCycle > 0) {
                currentCycleEvents.add(ALERT + ": Dropped " + droppedEventsThisCycle + " events (cap reached)");
            }
