...
public class PostMatchSummary {
    ...
    /**
     * Start tracking match data.
     * Call in autonomousInit() or teleopInit().
     */
    ...
    /**
     * Update tracking with current robot state.
     * Call every cycle in robotPeriodic() when enabled.
     */
     ...
     * Call in disabledInit() after a match.
     */
    ...
    private void logSummary(int healthScore) {
        ...
        for (int i = 0; i < 3; i++) {
            String val = (i < ranked.size())
                ? String.format("[%d] %.1fs: %s", ranked.get(i).severity,
                    ranked.get(i).timestampSec, ranked.get(i).description)
                : "";
    ...
    private void sendNotification(int healthScore) {
        ...
        if (!issues.isEmpty()) {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                sb.append(String.format("\n%d. [%.1fs] %s", i + 1,
                        issue.timestampSec, issue.description));
            }
        }
