...
/**
 * Centralized alert management for robot health monitoring.
 * Monitors battery, motor temps, CAN bus, and loop timing.
 * Elastic notifications are debounced to prevent spam.
 */
public class AlertManager {
    ...
    private final Alert batteryCriticalAlert = new Alert("BATTERY CRITICAL - Replace now!", AlertType.kError);
    ...
    private final Alert intakeActuatorTempAlert = new Alert("IntakeActuator motor overheating", AlertType.kWarning);
    private final Alert canUtilizationAlert = new Alert("CAN bus utilization high", AlertType.kWarning);
    private final Alert canBusOffAlert = new Alert("CAN bus error detected", AlertType.kError);
    private final Alert loopTimeWarningAlert = new Alert("Loop time exceeding 20ms", AlertType.kWarning);
    ...
    private final Alert bandwidthWarningAlert = new Alert("Network bandwidth > 70%", AlertType.kWarning);
    private final Alert bandwidthCriticalAlert = new Alert("Network bandwidth > 90% - REDUCE CAMERA!", AlertType.kError);

    private final Alert brownoutRiskAlert = new Alert("BROWNOUT IMMINENT - reduce power!", AlertType.kError);
    ...
    /**
     * Runs all alert checks except loop time.
     * Loop time checked separately via checkLoopTime(). Call logActiveAlerts() after both.
     */
    ...
    public void logActiveAlerts() {
        ...
        Logger.recordOutput("Alerts/ActiveList", activeAlerts.isEmpty() ? "none" : String.join(", ", activeAlerts));
    }
    ...
    private void checkBattery() {
        ...
        if (voltage < BATTERY_CRITICAL_V) {
            ...
            notifyElastic("BatteryCritical", "Battery Critical",
                    String.format("Battery at %.1fV - REPLACE NOW!", voltage), true);
        } else if (voltage < BATTERY_WARNING_V) {
            batteryCriticalAlert.set(false);
            batteryLowAlert.set(true);
            addAlert("BatteryLow");
            notifyElastic("BatteryLow", "Battery Low",
                    String.format("Battery at %.1fV", voltage), false);
        } else {
            batteryCriticalAlert.set(false);
            batteryLowAlert.set(false);
        }
    ...
    private void checkBrownoutRisk() {
        try {
            ...
            if (tm.isBrownoutRisk()) {
                ...
                notifyElastic("BrownoutRisk", "BROWNOUT IMMINENT",
                        String.format("Voltage dropping fast at %.1fV - reduce power!",
                            RobotController.getBatteryVoltage()), true);
            } else {
            ...
            if (shooter != null) {
                ...
                if (shooterTemp > MOTOR_TEMP_WARNING_C) {
                    ...
                    if (shooterTemp > MOTOR_TEMP_CRITICAL_C) {
                        notifyElastic("ShooterOverheat", "Shooter Overheating",
                                String.format("Shooter at %.0f°C - CRITICAL!", shooterTemp), true);
                    }
                } else {
    ...
    private void checkMotorStalls() {
        try {
            ...
            if (tm.isShooterStalled()) {
                ...
                notifyElastic("ShooterStall", "Shooter Stalled",
                        "Shooter motor stalled - check for obstruction", true);
            } else {
            ...
            if (tm.isIndexerStalled()) {
                ...
                notifyElastic("IndexerStall", "Indexer Stalled",
                        "Indexer motor stalled - check for obstruction", true);
            } else {
            ...
            if (tm.isIntakeStalled()) {
                ...
                notifyElastic("IntakeStall", "Intake Stalled",
                        "Intake motor stalled - check for obstruction", true);
            } else {
    ...
    private void checkCANBus() {
        ...
        if (canStatus.busOffCount > 0) {
            ...
            addAlert("CANBusOff");
            notifyElastic("CANBusOff", "CAN Bus Error",
                    "CAN bus off detected - check wiring!", true);
    ...
    private void checkJams() {
        if (TelemetryManager.getInstance().isIndexerJamDetected()) {
            ...
            addAlert("IndexerJam");
            notifyElastic("IndexerJam", "Indexer Jam",
                    "Indexer jam detected - clear manually", true);
        } else {
        ...
        if (TelemetryManager.getInstance().isIntakeJamDetected()) {
            ...
            addAlert("IntakeJam");
            notifyElastic("IntakeJam", "Intake Jam",
                    "Intake jam detected - clear manually", true);
        } else {
    ...
    private void checkBandwidth() {
        ...
        if (tm.isBandwidthCritical()) {
            ...
            notifyElastic("BandwidthCritical", "Bandwidth Critical",
                    String.format("%.0f%% - REDUCE CAMERA RESOLUTION!", tm.getBandwidthPercent()), true);
