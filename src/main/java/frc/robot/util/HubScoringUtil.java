...
public class HubScoringUtil {
  ...
  public static Pose2d getClosestScoringPose(
    ...
    Translation2d hubToRobot =
        robotPosition.minus(hubCenter); // calculates line from center of hub to the robot

    double angleToRobot =
        Math.atan2(
            hubToRobot.getY(), hubToRobot.getX()); // calculates the angle of that line on a circle

    double centerAngle = scoringSide.getRadians(); // gets the angle that the arc is around
    double halfArcRadians =
        Math.toRadians(
            arcWidthDegrees
                / 2.0); // splits the angle in half, both +-1/2totalangle around the center of the
    // angle.
