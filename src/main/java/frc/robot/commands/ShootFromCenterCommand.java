...
public class ShootFromCenterCommand extends Command {
  ...
  public ShootFromCenterCommand(
      SwerveSubsystem swerve, Shooter shooter, Indexer indexer, Agitator agitator) {
    this.swerve = swerve;
  ...
  public void execute() {
    ...
    SmartDashboard.putNumber(
        "ShootCenter/HeadingErrorDeg", Math.toDegrees(headingController.getPositionError()));
  }
  ...
  private Rotation2d computeAllianceSideFacing() {
    ...
    Rotation2d facingOurSide = isRed ? Rotation2d.fromDegrees(180) : Rotation2d.fromDegrees(0);
  ...
  private Translation2d getHubCenter() {
    boolean isRed = DriverStation.getAlliance().orElse(Alliance.Blue) == Alliance.Red;
    return isRed ? HubScoringConstants.RED_HUB_CENTER : HubScoringConstants.BLUE_HUB_CENTER;
  }
