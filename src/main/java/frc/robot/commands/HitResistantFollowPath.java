...
public class HitResistantFollowPath extends Command {
    ...
    public HitResistantFollowPath(SwerveSubsystem swerve,
        ...
        this.mainController = new PPHolonomicDriveController(
    ...
    public void execute() {
        ...
        virtualTime = Math.min(
            virtualTime + realDt * ghostBotScale,
        ...
        // cross track error
        Translation2d errorVector = ghostState.pose.getTranslation().minus(currentPose.getTranslation());
        ...
        SmartDashboard.putNumber("HitResistant/TrajRemaining",
            trajectory.getTotalTimeSeconds() - virtualTime);

        if (followState != lastLoggedState) {
            System.out.println("[HitResistant] " + lastLoggedState + " -> " + followState
                + " | error=" + String.format("%.3fm", poseError)
                + " | t="    + String.format("%.2fs", virtualTime));
    ...
    private void executeNominal(Pose2d currentPose,
                                PathPlannerTrajectoryState ghostState,
                                double poseError,
                                double now) {

        boolean startupSuppressed = (now - startTimestamp) < RECOVERY_SUPPRESSION_TIME;
        boolean cooldownActive = reentryExitTimestamp > 0
            && (now - reentryExitTimestamp) < POST_REENTRY_COOLDOWN;
    ...
    private void executeReentry(Pose2d currentPose,
                                PathPlannerTrajectoryState ghostState,
                                double poseError,
                                double realDt) {
    ...
    private double findClosestTrajectoryTime(Pose2d currentPose) {
        ...
        for (double t = virtualTime; t <= maxSearchTime; t += 0.05) {
            double dist = trajectory.sample(t).pose.getTranslation()
    ...
    private Command buildRecoverySubCommand(Pose2d startPose,
                                            PathPlannerTrajectoryState interceptState) {
        PathConstraints constraints = new PathConstraints(
            RECOVERY_MAX_VEL, RECOVERY_MAX_ACCEL,
            RECOVERY_MAX_OMEGA, RECOVERY_MAX_ALPHA
        );

        Rotation2d departureHeading = new Rotation2d(
        ...
        GoalEndState goalEndState = new GoalEndState(
            interceptState.linearVelocity,
            interceptState.pose.getRotation()
        );
        ...
        PPHolonomicDriveController recoveryCtrl = new PPHolonomicDriveController(
            new PIDConstants(5.0, 0.0, 0.0),
            new PIDConstants(5.0, 0.0, 0.0)
        );
        ...
        return new Command() {
            ...
            public void execute() {
                ...
                ChassisSpeeds speeds = recoveryCtrl.calculateRobotRelativeSpeeds(
                    swerve.getPose(), recoveryTraj.sample(t));
                swerve.drive(speeds);
            }
    ...
    private void buildEventSchedule() {
        ...
        for (EventMarker marker : markers) {
            ...
            if (cmd != null) {
                eventSchedule.add(new ScheduledEvent(triggerTime, cmd,
                    "event@t=" + String.format("%.2f", triggerTime)));
            }
        }
    ...
    private double arcLengthFractionToTime(double fraction, double totalTime) {
        ...
        for (int i = 1; i < samples; i++) {
            ...
            cumArc[i] = cumArc[i - 1] +
                prev.pose.getTranslation().getDistance(curr.pose.getTranslation());
            prev = curr;
        }
        ...
        while (hi - lo > 1) {
            int mid = (lo + hi) / 2;
            if (cumArc[mid] < targetLen) lo = mid; else hi = mid;
        }

        double span = cumArc[hi] - cumArc[lo];
        double t    = (span < 1e-9)
            ? lo * dt
            : (lo + (targetLen - cumArc[lo]) / span) * dt;
    ...
    public double      getGhostBotScale() { return ghostBotScale; }
