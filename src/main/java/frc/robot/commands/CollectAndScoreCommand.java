...
public class CollectAndScoreCommand extends SequentialCommandGroup {
  ...
  /**
   * Dynamically generates a PathPlanner path from the robot's current pose back to the scoring
   * position in front of the hub.
   */
  private Command buildReturnToHubCommand() {
    ...
    List<Waypoint> waypoints = PathPlannerPath.waypointsFromPoses(currentPose, hubScoringPose);

    // If we're already very close to the hub, skip the path
    double distance = currentPose.getTranslation().getDistance(hubScoringPose.getTranslation());
  ...
  /**
   * Returns the scoring pose in front of the hub based on alliance color. Uses SCORING_DISTANCE
   * from hub center and faces the robot toward the hub.
   */
  private Pose2d getHubScoringPose() {
    boolean isRed = DriverStation.getAlliance().orElse(Alliance.Blue) == Alliance.Red;

    Translation2d hubCenter =
        isRed ? HubScoringConstants.RED_HUB_CENTER : HubScoringConstants.BLUE_HUB_CENTER;

    // Position the robot in front of the hub on the alliance's scoring side
    Rotation2d facingHub =
        isRed ? HubScoringConstants.RED_SCORING_SIDE : HubScoringConstants.BLUE_SCORING_SIDE;
    ...
    Translation2d scoringPosition =
        new Translation2d(hubCenter.getX() + offsetX, hubCenter.getY() + offsetY);
