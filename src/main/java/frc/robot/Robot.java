...
public class Robot extends LoggedRobot {
  ...
  // MapleSim is auto-configured by YAGSL when SwerveDriveTelemetry.isSimulation=true.
  // Physics run inside SwerveDrive.updateOdometry() → SimulatedArena.simulationPeriodic().
  ...
  private void configureLogging() {
    try {
      ...
      if (isReal()) {
        // USB logging - optional, continue if no USB
        try {
  ...
  public void robotPeriodic() {
    ...
    try {
      DriverFeedback.getInstance().update();
    } catch (Throwable t) {
      safeLog("Health/CrashBarrier/DriverFeedback", true);
      safeLog("Health/CrashBarrier/LastError", t.getClass().getSimpleName());
    }
    LoggedTracer.record("HapticMs");
  ...
  public void simulationInit() {
    // YAGSL auto-configures MapleSim when SwerveDriveTelemetry.isSimulation=true.
    // No fallback needed, MapleSim is always available in simulation.
    safeLog("Sim/Initialized", true);
  }

  /** This function is called periodically whilst in simulation. */
  @Override
  public void simulationPeriodic() {
    // MapleSim physics run inside SwerveDrive.updateOdometry() via
    // SimulatedArena.simulationPeriodic(), do NOT call it here.
  }
}
