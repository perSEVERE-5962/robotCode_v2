...
public class DriverInputTelemetry implements SubsystemTelemetry {
  ...
  private double lastDriverInputTime = -1;
  ...
  public void update() {
    ...
    driverInputMagnitude =
        Math.sqrt(
            driverLeftX * driverLeftX
                + driverLeftY * driverLeftY
                + driverRightX * driverRightX
                + driverRightY * driverRightY);

    operatorInputMagnitude =
        Math.sqrt(operatorLeftX * operatorLeftX + operatorLeftY * operatorLeftY);

    driverActive =
        driverInputMagnitude > DEADBAND
  ...
  public double getDriverIdleTimeMs() {
    return driverIdleTimeMs;
  }

  private int packButtons(
      boolean a,
      boolean b,
      boolean x,
      boolean y,
