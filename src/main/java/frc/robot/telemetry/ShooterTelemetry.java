...
public class ShooterTelemetry implements SubsystemTelemetry {
    ...
    public void update() {
        ...
        boolean meetsStallCriteria = (currentAmps > StallDetectionConstants.SHOOTER_STALL_CURRENT_AMPS)
        ...
        shotDetected = (velocityDrop > shotDropThreshold)
        ...
        // Fire rate calculation (rolling window)
        while (!shotTimestamps.isEmpty() &&
               (now - shotTimestamps.peekFirst()) > FIRE_RATE_WINDOW_SEC) {
    ...
    public int getTotalShots() { return totalShotCount; }
    public double getVelocityRPM() { return velocityRPM; }
