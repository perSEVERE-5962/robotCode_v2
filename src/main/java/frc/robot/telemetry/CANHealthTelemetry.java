...
public class CANHealthTelemetry implements SubsystemTelemetry {
  private static final int TOTAL_DEVICES = 20; // 5 motors + gyro + 2 cameras + 12 swerve
  ...
  public void update() {
    ...
    // Per-swerve-module devices (4 modules x 3 devices each)
    for (int m = 0; m < 4; m++) {
      int base = 8 + m * 3;
    ...
    if (driveTelemetry != null) {
      for (int m = 0; m < 4; m++) {
        faults += driveTelemetry.getDriveFaultsRaw(m);
        faults += driveTelemetry.getTurnFaultsRaw(m);
      }
    }
    totalFaults = faults;
    hasFaults = (faults > 0);
