...
public class DriveTelemetry implements SubsystemTelemetry {
    ...
    public void log() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (moduleSetpoints[i] != null) {
                SafeLog.put("Drive/ModuleSetpoints/" + name + "/Angle",
                    moduleSetpoints[i].angle.getRadians());
                SafeLog.put("Drive/ModuleSetpoints/" + name + "/Speed",
    ...
    public double getPositionErrorMeters() { return positionErrorMeters; }
    public double getHeadingErrorDegrees() { return headingErrorDegrees; }
}
