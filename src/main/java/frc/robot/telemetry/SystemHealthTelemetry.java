...
public class SystemHealthTelemetry implements SubsystemTelemetry {
  ...
  // Per-channel PDH current logging (decimated to reduce CAN/log bandwidth)
  private static final int PDH_CHANNEL_DECIMATION = 5; // log every 5th cycle (~100ms at 50Hz)
  ...
  public void update() {
    ...
    try {
      ...
      if (pdh != null) {
        ...
        // Per-channel current: decimated to reduce CAN and log bandwidth
        ...
        if (pdhChannelCycleCounter >= PDH_CHANNEL_DECIMATION) {
          ...
          // Overcurrent alert: flag the highest channel if above threshold
  ...
  public void log() {
    ...
    // Per-channel PDH current (named by circuit for easy identification)
    ...
    // Overcurrent alert (single highest offending channel)
